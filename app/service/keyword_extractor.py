...
"""
이 모듈은 카페 리뷰에서 의미 있는 키워드를 추출하여 데이터베이스에 저장하는 기능을 제공합니다.
...
def extract_all_keywords():
    ...
    주요 기능:
    1. 기존 keywords 테이블의 데이터를 삭제하고 AUTO_INCREMENT를 초기화합니다.
    2. cafes 테이블에서 모든 카페 정보를 조회합니다.
    3. 각 카페별로 reviews 테이블에서 리뷰 내용을 가져와 형태소 분석을 수행합니다.
    4. 불용어 및 의미 없는 단어를 필터링하여 키워드를 선별합니다.
    5. 선별된 키워드의 빈도를 keywords 테이블에 갱신하거나 신규 삽입합니다.
